...
def main():
    ...
    if not st.session_state.authenticated:
        ...
        login_col = st.columns([3, 1, 3])[1]  # Centered login button
        with login_col:
            if st.button("Login"):
    ...
    if analysis_type == "Static":
        ...
        with st.container():
            ...
            with col1:
                x_col = st.selectbox("Select X-axis column", categorical_cols + numeric_cols, key="static_x")
                y_col = st.selectbox("Select Y-axis column", numeric_cols, key="static_y")
                color_col = st.selectbox("Select Color column (optional)", [None] + categorical_cols, key="static_color")
                if x_col and y_col:
        ...
        with st.container():
            ...
            if uploaded_file:
                try:
                    ...
                    x_col = st.selectbox("Select X-axis column (Uploaded Data)", user_df.columns, key="dynamic_x")
                    y_col = st.selectbox("Select Y-axis column (Uploaded Data)", user_df.select_dtypes(include=[np.number]).columns, key="dynamic_y")
                    if x_col and y_col:
